...
4. 題型設計原則 (統一造句軸)
   ...
   - 變化維度：自由度 × WALS 規則約束強度。
     - L1 詞塊填空：例句已給，一個空格，詞塊池 4 選 1（最低自由度，無 WALS）
     - L2 詞塊重組：正確句子打亂，還原語序（低自由度，WALS 提示但不計分）
     - L3 詞塊造句：自由組句，目標詞必現（高自由度，WALS 計分）—— 基準軸
     - L4 限制造句：自由組句，必須滿足 N 條已解鎖 WALS 規則才過關（高自由度，WALS 強制門檻）
     - L5 情境造句：給定語境提示句，組句回應（最高自由度，WALS 計分 + 情境加成）
   - 音訊維度：為跨級修飾器（TTS 播放 + 口說驗證），不獨立成題型類別。
     - L1 音訊版：TTS 播放例句，詞塊不顯示文字，靠聽覺選出目標詞
     - L1-S：填空完成後覆誦整句，SpeechRecognition 比對目標詞發音
     - L2 音訊版：TTS 播放原句，依記憶重組詞塊（取代現有 L2-A 手寫填空）
   - 拆解現有模組：L1-A（聽音多選）、L1-S（單獨覆誦）、L2-A（手寫填空）均折入上述音訊維度，不再作為獨立題型類別。
